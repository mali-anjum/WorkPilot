...
public class JobIngestionTests(SharedApiFactory factory)
{
    ...
    public async Task Ingest_StoresCanonicalJobsWithProvenanceSnapshotsAndOneAuditRow()
    {
        ...
        try
        {
            ...
            Assert.Equal(new JobIngestionSummary(sourceId, 2, 2, 2, 0, 0, 0), summary);

            await using var db = CreateDbContext();
            var jobs = await db.Jobs.Include(j => j.Snapshots).Where(j => j.JobSourceId == sourceId).OrderBy(j => j.ExternalId).ToListAsync();
            Assert.Equal(["1", "2"], jobs.Select(j => j.ExternalId));
            Assert.All(jobs, j =>
            {
                Assert.Equal($"https://boards.test/jobs/{j.ExternalId}", j.Provenance.SourceUrl);
                ...
                Assert.Equal(j.ExternalId, snapshot.ExternalId);
    ...
    public async Task Ingest_AppliesTheKeywordFilterAndSkipsUnusablePostings()
    {
        ...
        try
        {
            ...
            Assert.Equal(new JobIngestionSummary(sourceId, 4, 2, 1, 0, 0, 1), summary);

            var summaryAll = await IngestAsync(new ScriptedSource(Posting("3", "  ")), sourceId, keywords: null, T0);
            Assert.Equal(new JobIngestionSummary(sourceId, 1, 1, 0, 0, 0, 1), summaryAll);

            await using var db = CreateDbContext();
            Assert.Equal(1, await db.Jobs.CountAsync(j => j.JobSourceId == sourceId));
        }
        finally
        {
            await CleanupAsync(sourceId);
    ...
    public async Task Ingest_TwiceIsIdempotent_AndChangedContentAddsASnapshot()
    {
        ...
        try
        {
            ...
            Assert.Equal(new JobIngestionSummary(sourceId, 2, 2, 0, 0, 2, 0), again);
            ...
            Assert.Equal(new JobIngestionSummary(sourceId, 2, 2, 0, 1, 1, 0), changed);
    ...
    public async Task Ingest_WhenTheSourceFails_StoresNothing()
    {
        ...
        try
        {
            ...
            await using var db = CreateDbContext();
            Assert.Equal(0, await db.Jobs.CountAsync(j => j.JobSourceId == sourceId));
    ...
    public async Task ListJobs_ReturnsJobsWithProvenance_AndCapsTake()
    {
        ...
        try
        {
            ...
            Assert.All(all, j =>
            {
                Assert.Equal($"https://boards.test/jobs/{j.ExternalId}", j.SourceUrl);
    ...
    private static RawJobPosting Posting(string id, string title, string? location = "Berlin") =>
        new(id, $"https://boards.test/jobs/{id}", title, "Acme", location, "<p>Build it.</p>", T0.AddDays(-3), $"{{\"id\":{id}}}");
    ...
    private async Task<JobIngestionSummary?> IngestAsync(ScriptedSource source, Guid sourceId, string? keywords, DateTimeOffset now, bool fail = false)
    {
        source.Fail = fail;
        await using var db = CreateDbContext();
        var service = new JobIngestionService([source], new JobIngestionRepository(db), new AuditService(db), new FixedTime(now));
        return await service.IngestAsync(sourceId, keywords, CancellationToken.None);
    }
    ...
    private async Task CleanupAsync(Guid sourceId)
    {
        ...
        await db.JobSnapshots.Where(s => s.JobSourceId == sourceId).ExecuteDeleteAsync();
        await db.Jobs.IgnoreQueryFilters().Where(j => j.JobSourceId == sourceId).ExecuteDeleteAsync();
    ...
    private sealed record JobBody(string ExternalId, string SourceUrl, DateTimeOffset RetrievedAt, DateTimeOffset? VerifiedAt, int SnapshotCount);
}
