...
## Commands
- [x] Default config (no AI env vars): startup logs a warning that `Default` and `Planner` use the Fake provider; `GET /health/ai` → `200`, `provider: "Fake"` → AC-3, AC-8
- [ ] `Default` → `openai` / `gpt-4o-mini`: `GET /health/ai` → `200`, `provider: "openai"`, `model: "gpt-4o-mini"`, a real `latencyMs` → AC-2, AC-8
- [ ] Same for `gemini` / `gemini-3.8-flash` (Google retired `gemini-2.5-flash` for new keys), `deepseek` / `deepseek-chat`, `ollama` / `qwen2.5:7b` (a code change between them: none) → AC-1, AC-2
...
- [x] `Planner` → `deepseek`, `Default` → `openai`: a run plans through DeepSeek (its span shows `deepseek-chat`) while `/health/ai` reports `openai` → AC-1
- [x] Remove the `deepseek` key while `Default` uses it → the Api refuses to start, and the error names `Ai:Providers:deepseek:ApiKey` without printing any key → AC-4
- [x] Set `Ai__Purposes__Planer__Provider=openai` (typo) → startup fails naming `Ai:Purposes:Planer` → AC-4
- [x] Set the `openai` key to a wrong value → `/health/ai` → `503` with a 401 error and no key text; trigger a run → `Failed`, one `PlanningFailed` audit row with `reason: provider_error`, provider `openai`, no key or goal text → AC-5, AC-6
- [x] Point `ollama` at a stopped server (or a bad port) with `TimeoutSeconds: 1` → the run fails within about 3 × 1s plus backoff, audited as `provider_error` → AC-5, AC-6
...
- [x] `PlanningFailed.provider` is the config name (`deepseek`), not the SDK's `openai` → AC-6
- [x] `/health/ai` `purpose` is always `Default` → AC-8
